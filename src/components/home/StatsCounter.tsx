...
function AnimatedNumber({ target, suffix }: { target: number; suffix: string }) {
  ...
  return (
    <div ref={ref} className="text-white text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight">
      {display}{suffix}
    </div>
  );
}
...
export default function StatsCounter() {
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[1440px] mx-auto px-4 md:px-6 py-14 md:py-20">
        <div className="flex flex-wrap justify-center items-center">
          {stats.map((stat, i) => (
            <div key={stat.label} className="flex items-center">
              <div className="flex flex-col items-center text-center px-6 md:px-10 lg:px-14 py-4">
                <AnimatedNumber target={stat.value} suffix={stat.suffix} />
                <p className="text-white/70 text-sm md:text-base font-medium mt-2 tracking-wide">
                  {stat.label}
                </p>
              </div>
              {i < stats.length - 1 && (
